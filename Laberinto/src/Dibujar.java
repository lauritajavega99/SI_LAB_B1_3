...
public class Dibujar {
	...
	public void dibujar(Laberinto lab, int N) {
		// System.out.print("EJEMPLO" + lab.getColumnas() +
		// lab.getCeldas()[0][0].getVecinos()[0]);
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				...
				if (lab.getCeldas()[flab][j].getVecinoN()) {
					// System.out.print("BORRO " + "NORTE" + " "+
					// lab.getCeldas()[flab][j].getPosicion()[0]+"
					// "+lab.getCeldas()[flab][j].getPosicion()[1]+"\n");
					// System.out.println((j+1)+" "+fd);
					miLaberinto.borrar((j + 1), fd, 'N');
				}
				if (lab.getCeldas()[flab][j].getVecinoE()) {
					// System.out.print("BORRO " + "ESTE" + "
					// "+lab.getCeldas()[flab][j].getPosicion()[0]+"
					// "+lab.getCeldas()[flab][j].getPosicion()[1]+"\n");
					// System.out.println((j+1)+" "+fd);
					miLaberinto.borrar((j + 1), fd, 'E');
				}
				if (lab.getCeldas()[flab][j].getVecinoS()) {
					// System.out.print("BORRO " + "SUR" + "
					// "+lab.getCeldas()[flab][j].getPosicion()[0]+"
					// "+lab.getCeldas()[flab][j].getPosicion()[1]+"\n");
					// System.out.println((j+1)+" "+fd);
					miLaberinto.borrar((j + 1), fd, 'S');
				}
				if (lab.getCeldas()[flab][j].getVecinoO()) {
					// System.out.print("BORRO " + "OESTE" + "
					// "+lab.getCeldas()[flab][j].getPosicion()[0]+"
					// "+lab.getCeldas()[flab][j].getPosicion()[1]+"\n");
					// System.out.println((j+1)+" "+fd);
